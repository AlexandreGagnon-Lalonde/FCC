...
// Use the map Method to Extract Data from an Array
  ...
  var ratings = [];
  watchList.map(obj => {
    ratings.push({title: obj.Title, rating: obj.imdbRating})
  })
...
// Use the reduce Method to Analyze Data
  ...
  function getRating(watchList){
    // Only change code below this line
    let count = 0;
    var averageRating = watchList.reduce((ave, film) => {
      if (film.Director === 'Christopher Nolan') {
        count++;
        return ave + parseFloat(film.imdbRating)
      }
      return ave
    }, 0) / count;
...
// Use Higher-Order Functions map, filter, or reduce to Solve a Complex Problem
  const squareList = (arr) => {
    ...
    const squaredArr = filteredArr.map(element => element ** 2)
    return squaredArr;
    // Only change code above this line
  };
